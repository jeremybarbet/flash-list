...
class AutoLayoutShadow {
  ...
  var experimentalMaintainVisibleContentPosition = false
   ...
   * (Only Grid layouts with varying spans). Performance: RecyclerListView renders very
   * small number of views and this is not going to trigger multiple layouts on Android side.
   * Not expecting any major perf issue.
   */
  fun clearGapsAndOverlaps(sortedItems: Array<CellContainer>, scrollView: ScrollView) {
    ...
    for (index in 0 until sortedItems.size - 1) {
      val cell = sortedItems[index]
      val neighbour = sortedItems[index + 1]
      ...
      /**
       * This could be improved so when initializing a FlashList and adding new item to the data
       * straight away, we stay focus on the item visible currently on the screen.
       *
       * At the moment, if the user doesn't do anything after entering the screen and new item
       * is added to the data array, it will constantly shift focus to the new added item.
       */
      val isAnchorFound =
        !anchorSet && (nextAnchorStableId.isEmpty() || neighbour.stableId == anchorStableId)

      if (experimentalMaintainVisibleContentPosition && isAnchorFound) {
        nextAnchorOffset = if (horizontal) neighbour.left else neighbour.top
        nextAnchorStableId = neighbour.stableId
        anchorSet = true
      }

      lastMaxBoundOverall =
        kotlin.math.max(lastMaxBoundOverall, if (horizontal) cell.right else cell.bottom)

      lastMaxBoundOverall =
        kotlin.math.max(lastMaxBoundOverall, if (horizontal) neighbour.right else neighbour.bottom)
    }
    ...
    if (experimentalMaintainVisibleContentPosition) {
      for (cell in sortedItems) {
        ...
        if (cell.stableId == anchorStableId && minValue != anchorOffset) {
          val diff = minValue - anchorOffset

          (scrollView as BidirectionalScrollView).setShiftOffset(diff.toDouble())
  ...
  /**
   * Offset provided by react can be one frame behind the real one, it's important
   * that this method is called with offset taken directly from scrollview object.
   */
  fun computeBlankFromGivenOffset(
    actualScrollOffset: Int,
    distanceFromWindowStart: Int,
    distanceFromWindowEnd: Int
  ): Int {
    ...
    blankOffsetAtEnd =
      actualScrollOffset + windowSize - renderOffset - lastMaxBound - distanceFromWindowEnd

    return kotlin.math.max(blankOffsetAtStart, blankOffsetAtEnd)
  }

  /**
   * It's important to avoid correcting views outside the render window. An item that isn't
   * being recycled might still remain in the view tree. If views outside get considered
   * then gaps between unused items will cause algorithm to fail.
   */
