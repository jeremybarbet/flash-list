...
/**
 * Container for all RecyclerListView children. This will automatically remove all gaps
 * and overlaps for GridLayouts with flexible spans. Note: This cannot work for masonry
 * layouts i.e, pinterest like layout
 */
class AutoLayoutView(context: Context) : ReactViewGroup(context) {
  ...
  var experimentalMaintainVisibleContentPosition: Boolean = false
    set(value: Boolean) {
      alShadow.experimentalMaintainVisibleContentPosition = value
      field = value
    }
  ...
  /**
   * Overriding draw instead of onLayout. RecyclerListView uses absolute positions for each and
   * every item which means that changes in child layouts may not trigger onLayout on this
   * container. The same layout can still cause views to overlap. Therefore, it makes sense
   * to override draw to do correction.
   */
  override fun dispatchDraw(canvas: Canvas) {
    ...
    if (enableInstrumentation && parentScrollView != null) {
      /**
       * Since we need to call this method with scrollOffset on the UI thread and not with
       * the one react has we're querying parent's parent directly which will be a ScrollView.
       * If it isn't reported values will be incorrect but the component will not break.
       * RecyclerListView is expected not to change the hierarchy of children.
       */

      val scrollContainerSize =
        if (alShadow.horizontal) parentScrollView.width else parentScrollView.height

      val scrollOffset =
        if (alShadow.horizontal) parentScrollView.scrollX else parentScrollView.scrollY
      ...
      alShadow.computeBlankFromGivenOffset(
        scrollOffset,
   ...
   * Performance: Sort is needed. Given relatively low number of views in RecyclerListView
   * render tree this should be a non issue.
   */
  ...
  private fun fixFooter() {
    ...
    val isAutoLayoutEndVisible =
      if (alShadow.horizontal) right <= parentScrollView.width else bottom <= parentScrollView.height
