 ...
 * all gaps and overlaps for GridLayouts with flexible spans.
...
class AutoLayoutView(context: Context) : ReactViewGroup(context) {
  ...
  var maintainVisibleContentPosition: Boolean = false
  ...
  /**
   * Overriding draw instead of onLayout. RecyclerListView uses absolute positions for each
   * and every item which means that changes in child layouts may not trigger onLayout on
   * this container. The same layout can still cause views to overlap. Therefore, it makes
   * sense to override draw to do correction.
   */
   ...
   * Performance: Sort is needed. Given relatively low number of views
   * in RecyclerListView render tree this should be a non issue.
   */
