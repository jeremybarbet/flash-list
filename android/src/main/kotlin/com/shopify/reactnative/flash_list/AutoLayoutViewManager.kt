...
/**
 * ViewManager for AutoLayoutView - Container for all RecyclerListView children. Automatically
 * removes all gaps and overlaps for GridLayouts with flexible spans. Note: This cannot work
 * for masonry layouts i.e, pinterest like layout
 */
@ReactModule(name = AutoLayoutViewManager.REACT_CLASS)
class AutoLayoutViewManager : ViewGroupManager<AutoLayoutView>(),
  AutoLayoutViewManagerInterface<AutoLayoutView> {
  ...
  override fun createViewInstance(context: ThemedReactContext): AutoLayoutView {
    return AutoLayoutView(context).also {
      it.pixelDensity = context.resources.displayMetrics.density.toDouble()
    }
  }
  ...
  @ReactProp(name = "experimentalMaintainVisibleContentPosition")
  override fun setExperimentalMaintainVisibleContentPosition(
    view: AutoLayoutView,
    experimentalMaintainVisibleContentPosition: Boolean
  ) {
    view.experimentalMaintainVisibleContentPosition = experimentalMaintainVisibleContentPosition
  }
