...
@objc public class AutoLayoutView: UIView {
  ...
  @objc func setMaintainVisibleContentPosition(_ maintainVisibleContentPosition: Bool) {
    self.maintainVisibleContentPosition = maintainVisibleContentPosition
  }
  ...
  func getScrollViewOffset(for scrollView: UIScrollView?) -> CGFloat {
    ...
    if let scrollView, maintainVisibleContentPosition {
      return horizontal ?
  ...
  private func adjustTopContentPosition(
    ...
    for cellContainer in cellContainers {
      let minValue = horizontal ? cellContainer.frame.minX : cellContainer.frame.minY

      print("anchorStableId \(anchorStableId)")
  ...
  private func clearGaps(for cellContainers: [CellContainerComponentView]) {
    ...
    cellContainers.indices.dropLast().forEach { index in
      ...
      let isAnchorFound = nextAnchorStableId == "" || nextCell.stableId == anchorStableId

      print("maintainVisibleContentPosition \(maintainVisibleContentPosition)")

      if maintainVisibleContentPosition && isAnchorFound {
